...
const DEFAULT_EVM_TRANSPORT = fallback([
  http(process.env.MANTLE_RPC || "https://rpc.mantle.xyz"),
], { rank: false });
...
function resolveChain(chainName?: string) {
  ...
  return { chain: mantle, rpc: process.env.MANTLE_RPC || "https://rpc.mantle.xyz" };
}
...
export function createChildWalletClient(
  ...
  return createWalletClient({
    ...
    chain,
    transport: rpcUrl ? http(rpcUrl) : DEFAULT_EVM_TRANSPORT,
  });
}
